...
def get_feishu_demo_status(session: Session = Depends(get_tenant_db)) -> DemoDataStatusRead:
...
def delete_feishu_demo_data(session: Session = Depends(get_tenant_db)) -> DemoDataStatusRead:
    try:
