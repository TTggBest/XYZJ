(() => {
  ...
  function dramaSortOrderOptions(current = "desc") {
    return [["desc", "倒序"], ["asc", "正序"]].map(([value, text]) => `<option value="${value}" ${current === value ? "selected" : ""}>${text}</option>`).join("");
  }
  ...
  document.addEventListener("click", async event => {
    ...
    try {
      if (action === "add-channel") openModal("新增频道", channelForm());
        ...
        state.dramaLibrarySortOrder = el("dramaLibrarySortOrder")?.value || "desc";
        ...
        state.dramaProgressSortOrder = el("dramaProgressSortOrder")?.value || "desc";
