(() => {
  ...
  async function showDramas() {
    ...
    const tableContent = rows.length
      ? table(["序号", "剧目", "批次", "到期时间", "剧集数", "合集总时长", "内容概述", "语言", "频道", "状态", ""], rows, 1480, "drama-library-table")
  ...
  async function showDramaProgress() {
    ...
    const rows = data.items.map(item => `<tr class="drama-progress-row ${dramaProgressRowClass(item)}" data-drama-progress-id="${item.drama_id}"><td class="serial-cell">${item.drama_number}</td><td class="drama-progress-title-cell"><span class="cell-main">${esc(item.chinese_title)}</span><span class="cell-sub mono">${esc(item.drama_code)}</span></td><td class="drama-progress-batch-cell">${compactDramaBatch(item.batch_name)}</td>${DRAMA_PRODUCTION_NODES.map(([field]) => `<td class="drama-progress-node-cell">${dramaProgressNodeCell(item, field)}</td>`).join("")}<td class="drama-progress-spec-cell"><span>${item.episode_count == null ? "—" : `${item.episode_count} 集`}</span><span class="mono">${formatDuration(item.total_duration_seconds)}</span></td><td class="drama-progress-overall-cell">${item.overall_status === "not_producing" ? tag("not_producing") : progress(item.progress_percent)}</td><td class="drama-progress-updated-cell">${dramaProgressUpdatedAt(item.updated_at)}</td><td class="drama-progress-actions-cell"><div class="row-actions"><button class="icon-button" title="编辑制剧进度" aria-label="编辑制剧进度" data-action="edit-drama-progress" data-id="${item.drama_id}" ${item.is_production_excluded ? "disabled" : ""}>${icon("pencil")}</button>${dramaProductionAction(item)}</div></td></tr>`);
