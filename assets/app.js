(() => {
  ...
  async function showDramas() {
    ...
    const rows = data.items.map(item => `<tr><td class="serial-cell">${item.sequence_number}</td><td><span class="cell-main">${esc(item.chinese_title)}</span><span class="cell-sub mono">${esc(item.drama_code)} · ${item.source_row_number ? `飞书第 ${item.source_row_number} 行` : "手工录入"}</span></td><td>${esc(item.batch_name || "—")}</td><td>${fmt(item.expires_at, true)}</td><td><span class="cell-summary">${esc(item.content_summary || "尚未录入")}</span></td><td>${item.language_count}</td><td>${item.published_channel_count}</td><td>${tag(item.status)}</td><td><button class="icon-button" title="查看剧目" aria-label="查看剧目" data-action="drama-detail" data-id="${item.id}">${icon("arrow-right")}</button></td></tr>`);
    ...
    const tableContent = rows.length
      ? table(["序号", "剧目", "批次", "到期时间", "内容概述", "语言", "频道", "状态", ""], rows, 1280, "drama-library-table")
  ...
  async function showDramaProgress() {
    ...
    const rows = data.items.map(item => `<tr data-drama-progress-id="${item.drama_id}"><td class="serial-cell">${item.sequence_number}</td><td><span class="cell-main">${esc(item.chinese_title)}</span><span class="cell-sub mono">${esc(item.drama_code)}</span></td><td>${esc(item.batch_name || "—")}</td>${DRAMA_PRODUCTION_NODES.map(([field]) => `<td>${tag(item[field])}</td>`).join("")}<td>${progress(item.progress_percent)}</td><td>${fmt(item.updated_at)}</td><td><button class="icon-button" title="编辑制剧进度" aria-label="编辑制剧进度" data-action="edit-drama-progress" data-id="${item.drama_id}">${icon("pencil")}</button></td></tr>`);
  ...
  document.addEventListener("click", async event => {
    ...
    try {
      if (action === "add-channel") openModal("新增频道", channelForm());
        ...
        notify(`飞书剧库同步完成：新增 ${result.rows_inserted}，更新 ${result.rows_updated}，跳过 ${result.rows_skipped}`);
        await loadView("dramas", { preservePosition: true });
      }
