(() => {
  ...
  const STATUS_LABELS = {
    ...
    not_started: "未开始", not_producing: "不制作", images_pending: "待出图"
  };
  ...
  function statusClass(value) {
    ...
    if (["pending", "pending_dispatch", "reserved", "draft", "partial", "partially_classified", "partially_generated", "waiting", "images_pending"].includes(value)) return "is-amber";
    return "";
  }
  ...
  function dramaStatusOptions(current = "active") {
    return [["active", "启用"], ["expired", "已到期"], ["blocked", "禁用"], ["archived", "已归档"]].map(([value, text]) => `<option value="${value}" ${current === value ? "selected" : ""}>${text}</option>`).join("");
  }
  function dramaSortOrderOptions(current = "asc") {
    return [["asc", "正序"], ["desc", "倒序"]].map(([value, text]) => `<option value="${value}" ${current === value ? "selected" : ""}>${text}</option>`).join("");
  }
  function dramaSortByOptions(current = "drama_number") {
    return [["drama_number", "序号"], ["comprehensive_score", "综合评分"]].map(([value, text]) => `<option value="${value}" ${current === value ? "selected" : ""}>${text}</option>`).join("");
  }
  function dramaPageSizeOptions(current = 50) {
    return [50, 100, 150].map(value => `<option value="${value}" ${Number(current) === value ? "selected" : ""}>${value} 条</option>`).join("");
  }
  async function showDramas() {
    const params = { page: state.dramaLibraryPage, page_size: state.dramaLibraryPageSize, sort_by: state.dramaLibrarySortBy, sort_order: state.dramaLibrarySortOrder, search: state.dramaLibrarySearch, status: state.dramaLibraryStatus, batch_name: state.dramaLibraryBatch };
    const data = await api(`/dramas/library${query(params)}`); state.dramaLibrary = data;
    const rows = data.items.map(item => `<tr class="drama-library-row ${item.status === "archived" ? "is-not-producing" : ""}"><td class="serial-cell">${item.drama_number}</td><td><span class="cell-main">${esc(item.chinese_title)}</span><span class="cell-sub mono">${esc(item.drama_code)} · ${item.source_row_number ? `飞书第 ${item.source_row_number} 行` : "手工录入"}</span></td><td>${item.comprehensive_score == null ? "—" : Number(item.comprehensive_score)}</td><td>${esc(item.batch_name || "—")}</td><td>${fmt(item.expires_at, true)}</td><td>${item.episode_count ?? "—"}</td><td class="mono">${formatDuration(item.total_duration_seconds)}</td><td><span class="cell-summary">${esc(item.content_summary || "尚未录入")}</span></td><td>${item.language_count}</td><td>${item.published_channel_count}</td><td>${tag(item.status)}</td><td><button class="icon-button" title="查看剧目" aria-label="查看剧目" data-action="drama-detail" data-id="${item.id}">${icon("arrow-right")}</button></td></tr>`);
    const filters = `<div class="drama-library-toolbar"><div class="field search-field"><label>搜索</label><input class="input" id="dramaLibrarySearch" value="${esc(state.dramaLibrarySearch)}" placeholder="剧名或剧库 ID"></div><div class="field"><label>状态</label><select class="select" id="dramaLibraryStatus"><option value="">全部状态</option>${dramaStatusOptions(state.dramaLibraryStatus)}</select></div><div class="field"><label>批次</label><input class="input" id="dramaLibraryBatch" value="${esc(state.dramaLibraryBatch)}" placeholder="全部批次"></div><div class="field"><label>排序字段</label><select class="select" id="dramaLibrarySortBy">${dramaSortByOptions(state.dramaLibrarySortBy)}</select></div><div class="field"><label>排序</label><select class="select" id="dramaLibrarySortOrder">${dramaSortOrderOptions(state.dramaLibrarySortOrder)}</select></div><div class="field"><label>每页</label><select class="select" id="dramaLibraryPageSize">${dramaPageSizeOptions(state.dramaLibraryPageSize)}</select></div><button class="button button-secondary" data-action="apply-drama-library-filters">${icon("search")} 查询</button><div class="toolbar-spacer"></div><button class="button button-secondary" data-action="go-drama-progress">${icon("workflow")} 制剧进度</button><button class="button button-secondary" data-action="sync-feishu-dramas">${icon("refresh-cw")} 同步飞书</button><button class="button button-secondary" data-action="bulk-add-dramas">${icon("file-up")} 批量录入</button><button class="button button-primary" data-action="add-drama">${icon("plus")} 新增剧目</button></div>`;
    const pager = data.pages > 1 ? `<div class="pagination"><button class="button button-secondary button-small" data-action="drama-library-page" data-page="${data.page - 1}" ${data.page <= 1 ? "disabled" : ""}>${icon("chevron-left")} 上一页</button><span>第 ${data.page} / ${data.pages} 页 · ${data.total} 条</span><button class="button button-secondary button-small" data-action="drama-library-page" data-page="${data.page + 1}" ${data.page >= data.pages ? "disabled" : ""}>下一页 ${icon("chevron-right")}</button></div>` : "";
    const tableContent = rows.length
      ? table(["序号", "剧目", "综合评分", "批次", "到期时间", "剧集数", "合集总时长", "内容概述", "语言", "频道", "状态", ""], rows, 1540, "drama-library-table")
      : empty("没有符合条件的剧目", "调整筛选条件，或从飞书同步剧库。");
    const libraryContent = `<div class="section-body drama-filter-body">${filters}</div>${tableContent}${pager}`;
    const summary = `${kpi("library", "总剧目", data.summary.total, "MySQL 剧目主档")}${kpi("circle-check", "启用", data.summary.active, "可用于新排期")}${kpi("calendar-clock", "30 天内到期", data.summary.expiring, "按北京时间计算")}${kpi("archive", "归档", data.summary.archived, "保留历史关系")}`;
  ...
  function compactDramaBatch(value) {
    const match = String(value || "").match(/\d+/);
    return match ? match[0] : "—";
  }
  function dramaProgressUpdatedAt(value) {
    const [date, ...timeParts] = fmt(value).split(/\s+/);
    const time = timeParts.join(" ");
    return `<span>${date}</span>${time ? `<span>${time}</span>` : ""}`;
  }
  ...
  async function showDramaProgress() {
    ...
    const rows = data.items.map(item => {
      const nodeCells = item.is_production_excluded
        ? `<td class="drama-progress-excluded-cell" colspan="${DRAMA_PRODUCTION_NODES.length}">${tag("not_producing")}</td>`
        : DRAMA_PRODUCTION_NODES.map(([field]) => `<td class="drama-progress-node-cell">${dramaProgressNodeCell(item, field)}</td>`).join("");
      return `<tr class="drama-progress-row ${dramaProgressRowClass(item)}" data-drama-progress-id="${item.drama_id}"><td class="serial-cell">${item.drama_number}</td><td class="drama-progress-title-cell"><span class="cell-main">${esc(item.chinese_title)}</span><span class="cell-sub mono">${esc(item.drama_code)}</span></td><td class="drama-progress-batch-cell">${compactDramaBatch(item.batch_name)}</td>${nodeCells}<td class="drama-progress-spec-cell"><span>${item.episode_count == null ? "—" : `${item.episode_count} 集`}</span><span class="mono">${formatDuration(item.total_duration_seconds)}</span></td><td class="drama-progress-overall-cell">${item.is_production_excluded ? "—" : progress(item.progress_percent)}</td><td class="drama-progress-updated-cell">${dramaProgressUpdatedAt(item.updated_at)}</td><td class="drama-progress-actions-cell"><div class="row-actions"><button class="icon-button" title="编辑制剧进度" aria-label="编辑制剧进度" data-action="edit-drama-progress" data-id="${item.drama_id}" ${item.is_production_excluded ? "disabled" : ""}>${icon("pencil")}</button>${dramaProductionAction(item)}</div></td></tr>`;
    });
    const filters = `<div class="drama-library-toolbar"><button class="button button-secondary" data-action="back-drama-library">${icon("arrow-left")} 返回剧库</button><div class="field search-field"><label>搜索</label><input class="input" id="dramaProgressSearch" value="${esc(state.dramaProgressSearch)}" placeholder="剧名或剧库 ID"></div><div class="field"><label>批次</label><input class="input" id="dramaProgressBatch" value="${esc(state.dramaProgressBatch)}" placeholder="全部批次"></div><div class="field"><label>整体状态</label><select class="select" id="dramaProgressStatus">${productionStatusOptions(state.dramaProgressStatus)}</select></div><div class="field"><label>当前节点</label><select class="select" id="dramaProgressNode">${productionNodeOptions(state.dramaProgressNode)}</select></div><div class="field"><label>排序</label><select class="select" id="dramaProgressSortOrder">${dramaSortOrderOptions(state.dramaProgressSortOrder)}</select></div><div class="field"><label>每页</label><select class="select" id="dramaProgressPageSize">${dramaPageSizeOptions(state.dramaProgressPageSize)}</select></div><button class="button button-primary" data-action="apply-drama-progress-filters">${icon("search")} 查询</button><button class="button button-secondary" data-action="sync-zhihe-progress">${icon("refresh-cw")} 同步智核</button></div>`;
  ...
  function dramaForm(drama = null) {
    ...
    const terms = (drama?.core_terms || []).map(item => item.term).join("，");
    return `<form class="form-grid" id="dramaForm"><input type="hidden" name="drama_id" value="${esc(drama?.id || "")}"><div class="field field-wide"><label>中文剧名</label><input class="input" name="chinese_title" value="${esc(drama?.chinese_title || "")}" required maxlength="255"></div><div class="field"><label>批次</label><input class="input" name="batch_name" value="${esc(drama?.batch_name || "")}" maxlength="120"></div><div class="field"><label>到期时间</label><input class="input" type="date" name="expires_at" value="${esc(dramaExpiryValue(drama?.expires_at))}"></div><div class="field field-wide"><label>百度网盘链接</label><input class="input" name="baidu_cloud_url" value="${esc(drama?.baidu_cloud_url || "")}" maxlength="1000"></div><div class="field field-wide"><label>内容概述</label><textarea class="textarea" name="content_summary">${esc(drama?.content_summary || "")}</textarea></div><div class="field field-wide"><label>剧情档案</label><textarea class="textarea textarea-tall" name="plot_archive">${esc(drama?.plot_archive || "")}</textarea></div><div class="field field-wide"><label>剧情套路</label><textarea class="textarea" name="plot_pattern">${esc(drama?.plot_pattern || "")}</textarea></div><div class="field field-wide"><label>核心人设</label><textarea class="textarea" name="core_personas">${esc(drama?.core_personas || "")}</textarea></div><div class="field field-wide"><label>别名（逗号分隔）</label><input class="input" name="aliases" value="${esc(aliases)}"></div><div class="field field-wide"><label>核心词（逗号分隔）</label><input class="input" name="core_terms" value="${esc(terms)}"></div><div class="field"><label>状态</label><select class="select" name="status">${dramaStatusOptions(drama?.status || "active")}</select></div><div class="form-actions"><button class="button button-secondary" type="button" data-close-modal>取消</button><button class="button button-primary" type="submit">保存剧目</button></div></form>`;
  }
  function dramaBulkForm() {
    return `<form class="form-grid" id="dramaBulkForm"><div class="field field-wide"><label>CSV 文件</label><input class="input" type="file" name="csv_file" accept=".csv,text/csv" required><span class="field-help">表头：作品名称、百度网盘链接、内容概述、剧情档案、剧情套路、核心人设、到期时间、批次、状态</span></div><div class="form-actions"><button class="button button-secondary" type="button" data-close-modal>取消</button><button class="button button-primary" type="submit">开始录入</button></div></form>`;
  }
  function progressEditForm(dramaId, productionState) {
    const options = current => [["not_started", "未开始"], ["in_progress", "进行中"], ["completed", "已完成"], ["failed", "失败"]].map(([value, text]) => `<option value="${value}" ${current === value ? "selected" : ""}>${text}</option>`).join("");
    return `<form class="form-grid" id="dramaProgressForm"><input type="hidden" name="drama_id" value="${esc(dramaId)}">${DRAMA_PRODUCTION_NODES.map(([field, text]) => `<div class="field"><label>${text}</label><select class="select" name="${field}">${options(productionState[field])}</select></div>`).join("")}<div class="field"><label>剧集数</label><input class="input" type="number" min="0" name="episode_count" value="${esc(productionState.episode_count ?? "")}"></div><div class="field"><label>合集总时长（秒）</label><input class="input" type="number" min="0" name="total_duration_seconds" value="${esc(productionState.total_duration_seconds ?? "")}"></div><div class="field field-wide"><label>失败原因</label><textarea class="textarea" name="last_error">${esc(productionState.last_error || "")}</textarea></div><div class="form-actions"><button class="button button-secondary" type="button" data-close-modal>取消</button><button class="button button-primary" type="submit">${icon("save")} 保存进度</button></div></form>`;
  }
  function dramaDetailBody(drama, tab) {
    const tabs = `<div class="detail-tabs"><button class="detail-tab ${tab === "basic" ? "is-active" : ""}" data-drama-tab="basic" data-action="drama-tab" data-id="${drama.id}">基本资料</button><button class="detail-tab ${tab === "production" ? "is-active" : ""}" data-drama-tab="production" data-action="drama-tab" data-id="${drama.id}">制剧进度</button><button class="detail-tab ${tab === "languages" ? "is-active" : ""}" data-drama-tab="languages" data-action="drama-tab" data-id="${drama.id}">语言覆盖</button><button class="detail-tab ${tab === "channels" ? "is-active" : ""}" data-drama-tab="channels" data-action="drama-tab" data-id="${drama.id}">频道分布</button></div>`;
    let body = "";
    if (tab === "languages") {
      const groups = ["S", "A", "B", "C"].map(tier => {
        const items = drama.languages.filter(item => item.priority_tier === tier);
        return `<section class="language-tier"><header><strong>${tier} 级语言</strong><span>${items.filter(item => item.translation_status !== "missing" || item.asset_status !== "missing").length} / ${items.length}</span></header><div class="language-grid">${items.map(item => { const covered = item.translation_status !== "missing" || item.asset_status !== "missing"; const feishuOwned = covered && item.source_type === "feishu"; return `<button class="language-toggle ${covered ? "is-covered" : ""}" ${feishuOwned ? "disabled title=\"飞书同步覆盖，请在飞书语言表修改\"" : `data-action="toggle-drama-language" data-id="${drama.id}" data-language-id="${item.language_id}" data-covered="${covered ? "1" : "0"}"`}><span>${esc(item.language_name_zh)}</span><span class="mono">${esc(item.language_code)}</span><strong>${covered ? "已完成" : "未完成"}</strong>${item.source_type ? `<small>${item.source_type === "feishu" ? "飞书" : "人工"}</small>` : ""}</button>`; }).join("")}</div></section>`;
      }).join("");
      ...
      body = `<div class="detail-actions"><button class="button button-primary button-small" data-action="edit-drama" data-id="${drama.id}">${icon("pencil")} 编辑</button></div><div class="detail-grid"><div class="detail-item"><span>剧库编号</span><strong class="mono">${esc(drama.drama_code)}</strong></div><div class="detail-item"><span>批次</span><strong>${esc(drama.batch_name || "—")}</strong></div><div class="detail-item"><span>到期时间</span><strong>${fmt(drama.expires_at, true)}</strong></div><div class="detail-item"><span>状态</span><strong>${tag(drama.status)}</strong></div><div class="detail-item"><span>来源</span><strong>${drama.source_type === "feishu" ? `飞书第 ${drama.source_row_number} 行` : "手工录入"}</strong></div><div class="detail-item"><span>最后同步</span><strong>${fmt(drama.source_synced_at)}</strong></div></div>${[["百度网盘链接", drama.baidu_cloud_url], ["内容概述", drama.content_summary], ["剧情档案", drama.plot_archive], ["剧情套路", drama.plot_pattern], ["核心人设", drama.core_personas]].map(([title, value]) => `<div class="detail-block"><h3>${title}</h3><p>${esc(value || "尚未录入")}</p></div>`).join("")}<div class="detail-block"><h3>别名</h3><p>${esc(drama.aliases.map(item => item.alias).join("、") || "—")}</p></div>`;
    }
    return `${tabs}<div class="drama-detail-content">${body}</div>`;
  }
  async function showDramaDetail(id, tab = "basic") {
    const drama = await api(`/dramas/${id}`); state.dramaDetail = drama; state.dramaDetailTab = tab;
    openDrawer(drama.chinese_title, dramaDetailBody(drama, tab));
  }

  function sourceVideoCell(item, entityType) {
    const targetId = entityType === "task" ? item.task_id : item.schedule_id;
    const link = item.source_video_url
      ? `<a class="cell-link mono" href="${esc(item.source_video_url)}" target="_blank" rel="noreferrer">${esc(item.source_video_id || "打开链接")}</a>`
      : `<span class="cell-sub mono">${esc(item.source_video_id || "待补充")}</span>`;
    return `<div class="row-actions">${link}<button class="button button-secondary button-small" data-action="edit-${entityType}-video" data-id="${targetId}">${icon("pencil")} 编辑</button></div>`;
  }

  function openSourceVideoForm(entityType, item) {
    const entityId = entityType === "task" ? item.task_id : item.schedule_id;
    openModal("编辑视频信息", `<form class="form-grid" id="sourceVideoForm"><input type="hidden" name="entity_type" value="${entityType}"><input type="hidden" name="entity_id" value="${entityId}"><div class="field"><label>剧目</label><input class="input" value="${esc(item.chinese_title)}" disabled></div><div class="field"><label>YouTube 链接</label><input class="input" name="source_video_url" value="${esc(item.source_video_url || "")}" placeholder="可留空，后续补充"></div><div class="field"><label>Video ID</label><input class="input mono" name="source_video_id" value="${esc(item.source_video_id || "")}" maxlength="32" placeholder="填链接时可自动识别"></div><div class="form-actions"><button class="button button-secondary" type="button" data-close-modal>取消</button><button class="button button-primary" type="submit">保存视频信息</button></div></form>`);
  }
  ...
  async function showSchedules() {
    ...
    const allDayMode = state.scheduleViewMode === "all-day";
    const schedules = !fullMode && (allDayMode || state.scheduleChannelId) ? await api(`/schedules/overview${query({ channel_id: allDayMode ? null : state.scheduleChannelId, date_from: date, date_to: date })}`) : [];
    ...
    const viewSwitch = `<div class="schedule-view-switch" role="group" aria-label="排期视图"><button class="button ${!fullMode && !allDayMode ? "button-primary" : "button-secondary"}" data-action="schedule-view-mode" data-mode="day">按日查看</button><button class="button ${fullMode ? "button-primary" : "button-secondary"}" data-action="schedule-view-mode" data-mode="full">频道完整排期</button><button class="button ${allDayMode ? "button-primary" : "button-secondary"}" data-action="schedule-view-mode" data-mode="all-day">全部频道当日</button></div>`;
    const commonActions = `<button class="button button-secondary" data-action="sync-feishu-channel-schedules">${icon("refresh-cw")} 同步飞书排期</button><button class="button button-secondary" data-action="go-publish-slots">${icon("clock-3")} 频道档期表</button>${allDayMode ? "" : `<button class="button button-primary" data-action="add-schedule" ${state.scheduleChannelId ? "" : "disabled"}>${icon("plus")} 新建排期</button>`}`;
    if (fullMode) {
      const rows = full.items.map(item => `<tr><td>${fmt(item.planned_beijing_time)}</td><td><span class="cell-main">${item.slot_type === "main" ? "主档" : "辅档"}</span><span class="cell-sub">档位 ${item.slot_number}</span></td><td><span class="cell-main">${esc(item.chinese_title)}</span><span class="cell-sub mono">${esc(item.drama_code)}</span></td><td>${sourceVideoCell(item, "schedule")}</td><td>${tag(item.is_uploaded ? "completed" : "pending")}</td><td>${tag(item.is_published ? "completed" : "pending")}</td><td>${tag(item.is_task_written ? "completed" : "pending")}</td><td>${item.source_type === "feishu" ? "飞书" : item.source_type === "system" ? "系统" : "人工"}</td><td>${fmtUtc(item.source_synced_at)}</td><td>${tag(item.schedule_status)}</td><td>${item.task_id ? tag(item.task_status) : "—"}</td></tr>`);
    ...
    const rows = schedules.map(item => `<tr>${allDayMode ? `<td><span class="cell-main">${esc(item.channel_name)}</span><span class="cell-sub mono">${esc(item.youtube_channel_id)}</span></td>` : ""}<td><span class="cell-main">${esc(item.chinese_title)}</span><span class="cell-sub">${esc(item.drama_code)}</span></td><td>${sourceVideoCell(item, "schedule")}</td><td>${esc(item.slot_type === "main" ? "主档" : "辅档")} ${String(item.slot_local_time).slice(0, 5)}</td><td>${fmt(item.planned_beijing_time)}</td><td>${esc(item.playlist_name || "—")}</td><td>${item.community_count}</td><td>${tag(item.schedule_status)}</td><td><div class="row-actions">${item.task_id ? tag(item.task_status) : `<button class="button button-quiet button-small" data-action="create-task" data-id="${item.schedule_id}">生成工单</button>`}</div></td></tr>`);
    const channelFilter = allDayMode ? "" : `<div class="field"><label>频道</label><select class="select" id="scheduleChannelFilter" ${channels.length ? "" : "disabled"}>${channelOptions || `<option>暂无频道</option>`}</select></div>`;
    const tools = `<div class="toolbar schedule-toolbar">${viewSwitch}${channelFilter}<div class="field"><label>排期日期</label><input class="input" type="date" id="scheduleDate" value="${date}"></div><div class="toolbar-spacer"></div>${commonActions}</div>`;
    const headers = [...(allDayMode ? ["频道"] : []), "剧目", "Video ID / 链接", "档位", "北京时间", "播放列表", "社群", "排期状态", "工单"];
    const title = allDayMode ? "全部频道当日排期" : "频道排期";
    const subtitle = allDayMode ? `${date} · ${schedules.length} 条安排 · 时间统一展示北京时间` : `${esc(selectedChannel?.display_name || "尚未选择频道")} · ${schedules.length} 条当日安排 · 时间统一展示北京时间`;
    const noData = allDayMode ? empty("当天所有频道都没有排期", "可切换到单频道当日视图创建排期。") : empty("该频道当天没有排期", "只能从制剧已全部完成的剧目中创建排期。", "add-schedule", "新建排期");
    root.innerHTML = `<div class="page-stack">${section(title, subtitle, rows.length ? table(headers, rows, allDayMode ? 1220 : 1080, "channel-schedule-table") : noData, tools)}</div>`;
  }
  ...
  async function openScheduleForm() {
    const channel = state.channels.find(item => item.channel_id === state.scheduleChannelId);
    if (!channel) return notify("请先选择频道", true);
    if (!state.dramas.length) return notify("当前没有制剧已全部完成的可排期剧目", true);
    openModal("新建频道排期", `<form class="form-grid" id="scheduleForm"><input type="hidden" name="channel_id" value="${channel.channel_id}"><div class="field"><label>频道</label><input class="input" value="${esc(channel.display_name)}" disabled></div><div class="field"><label>剧目（制剧已全部完成）</label><select class="select" name="drama_id" required><option value="">选择剧目</option>${state.dramas.map(d => `<option value="${d.id}">${esc(d.chinese_title)}</option>`).join("")}</select></div><div class="field"><label>发布日期</label><input class="input" type="date" name="publish_date" value="${state.date}" required></div><div class="field"><label>发布档位</label><select class="select" name="publish_slot_id" id="scheduleSlot" required disabled><option value="">正在读取档位</option></select></div><div class="field"><label>播放列表</label><select class="select" name="playlist_id" id="schedulePlaylist" disabled><option value="">不指定</option></select></div><div class="field"><label>社群数量</label><input class="input" type="number" name="community_count" value="0" min="0" max="20"></div><div class="field"><label>优先级</label><input class="input" type="number" name="priority" value="100" min="0"></div><div class="form-actions"><button class="button button-secondary" type="button" data-close-modal>取消</button><button class="button button-primary" type="submit">保存排期</button></div></form>`);
    await loadChannelScheduling(channel.channel_id);
  }
  ...
  async function showYoutube() {
    ...
    const videoRows = videos.map(v => { const drama = dramaById.get(v.drama_id); const binding = drama ? `<span class="cell-main">${esc(drama.chinese_title)}</span><span class="cell-sub mono">${esc(drama.drama_code)}</span>` : `<span class="tag">未关联排期</span>`; return `<tr><td><span class="cell-main">${esc(v.title || v.youtube_video_id)}</span><span class="cell-sub mono">${esc(v.youtube_video_id)}</span></td><td>${shortId(v.channel_id)}</td><td>${binding}</td><td>${tag(v.publish_status)}</td><td>${tag(v.privacy_status)}</td><td>${fmt(v.scheduled_publish_at || v.published_at)}</td></tr>`; });
    const unmatched = videos.filter(video => !video.drama_id).length;
    root.innerHTML = `<div class="page-stack"><div class="kpi-grid">${kpi("video", "视频", videos.length, "已落库 YouTube 视频")}${kpi("link", "未关联排期", unmatched, "等待频道排期或运营任务关联")}${kpi("message-square", "评论", comments.length, "默认排除频道自身评论")}${kpi("refresh-cw", "同步水位", watermarks.length, "按频道和数据类型隔离")}${kpi("gauge", "配额记录", quota.length, "数据库聚合")}</div>${section("YouTube 视频", "视频通过频道排期或运营任务关联剧目，不直接绑定公共剧库", videoRows.length ? table(["视频", "频道 ID", "排期剧目", "发布状态", "隐私状态", "发布时间"], videoRows, 980) : empty("还没有 YouTube 视频数据", "频道授权并同步后，视频数据会按频道落库。"))}</div>`;
  }

  function mediaAssetPosition(asset) {
    const source = `${asset.storage_key || ""}/${asset.original_filename || ""}`;
    const cover = source.match(/(?:02|04|06)_标题([123])_16x9(?:_logo)?\.[^/]+$/);
    if (cover) return { kind: "cover", number: Number(cover[1]), label: `封面${cover[1]}` };
    const community = source.match(/\d+_社群(\d+)_1x1\.[^/]+$/);
    if (community) return { kind: "community", number: Number(community[1]), label: `社群${community[1]}` };
    return { kind: "other", number: 999, label: asset.original_filename || "其他图片" };
  }

  function mediaFallbackMeta(asset) {
    const parts = String(asset.storage_key || "").split("/");
    const rootName = parts.includes("Logo成品") ? "Logo成品" : parts.includes("图片导入") ? "图片导入" : "";
    const rootIndex = rootName ? parts.indexOf(rootName) : -1;
    return {
      chinese_title: rootIndex >= 0 ? parts[rootIndex + 5] || asset.original_filename || "未命名素材" : asset.original_filename || "未命名素材",
      channel_name: rootIndex >= 0 ? parts[rootIndex + 3] || "未关联频道" : "未关联频道",
      batch_number: rootIndex >= 0 ? parts[rootIndex + 1] || "未分批" : "未分批",
      planned_local_time: rootIndex >= 0 ? parts[rootIndex + 4] || "未排期" : "未排期",
    };
  }
  ...
  function buildMediaGroups(assets, packages, channels) {
    ...
    packages.forEach(item => {
      ...
      const channel = channelById.get(item.channel_id) || channelByName.get(item.channel_name);
      groups.set(item.package_id, {
        key: item.package_id,
        meta: { ...item, language_code: item.language_code || channel?.default_language || "" },
        assets: [],
      });
    });
    assets.filter(asset => asset.asset_type === "image").forEach(asset => {
      ...
      groups.get(key).assets.push({ ...asset, ...mediaAssetPosition(asset), contentUrl: `/api/v3/media-assets/${encodeURIComponent(asset.id)}/content` });
    });
    return [...groups.values()].map(group => {
      group.assets.sort((left, right) => {
        const kindOrder = { cover: 0, community: 1, other: 2 };
        return kindOrder[left.kind] - kindOrder[right.kind] || left.number - right.number || String(left.original_filename || "").localeCompare(String(right.original_filename || ""));
      });
      group.createdAt = group.assets.reduce((latest, asset) => !latest || new Date(asset.created_at) > new Date(latest) ? asset.created_at : latest, null);
      const hasCoverage = Array.isArray(group.meta.expected_roles);
      group.complete = hasCoverage ? Boolean(group.meta.complete) : null;
      group.missingRoles = hasCoverage ? group.meta.missing_roles || [] : [];
      group.status = hasCoverage
        ? group.complete ? "ready" : "missing"
        : group.assets.every(asset => asset.status === "ready") ? "ready" : group.assets.some(asset => asset.status === "failed") ? "failed" : group.assets[0]?.status;
      return group;
    }).sort((left, right) => {
      const leftRow = Number(left.meta.source_row_number || 0);
      const rightRow = Number(right.meta.source_row_number || 0);
      if (leftRow && rightRow) return leftRow - rightRow;
      return new Date(right.createdAt || 0) - new Date(left.createdAt || 0);
    });
  }

  function mediaAssetCard(asset, groupIndex, assetIndex) {
    const thumbnailUrl = `/api/v3/media-assets/${encodeURIComponent(asset.id)}/thumbnail`;
    return `<article class="media-gallery-item"><button class="media-gallery-thumb" type="button" data-action="open-media-viewer" data-group-index="${groupIndex}" data-asset-index="${assetIndex}" aria-label="查看${esc(asset.label)}大图"><img src="${thumbnailUrl}" alt="${esc(asset.label)}" loading="lazy"></button><button class="media-gallery-name" type="button" data-action="reveal-media-asset" data-id="${esc(asset.id)}" title="打开图片所在文件夹">${icon("folder-open")}<span>${esc(asset.label)}</span></button></article>`;
  }

  function mediaGroupCard(group, groupIndex) {
    const meta = group.meta;
    const schedule = meta.planned_local_time || meta.target_publish_date || "未排期";
    const coverageClass = group.complete === true ? " is-complete" : group.complete === false ? " is-incomplete" : "";
    ...
    const assetsMarkup = group.assets.length
      ...
      : `<div class="media-gallery-empty">尚未导入图片 · 缺少 ${esc(group.missingRoles.join("、") || "全部图片")}</div>`;
    const lookupId = meta.video_id || meta.drama_code || meta.drama_id || "未登记";
    const lookupLabel = meta.video_id ? "Video ID" : "剧 ID";
    return `<article class="media-gallery-group${coverageClass}" data-media-package-id="${esc(meta.package_id || "")}"><div class="media-gallery-meta"><div><span>剧名 · 剧 ID</span><strong>${esc(meta.chinese_title || "未命名素材")}</strong><small class="media-video-id">${lookupLabel} · ${esc(lookupId)}</small></div><div><span>频道</span><strong>${esc(meta.channel_name || "未关联频道")}</strong></div><div><span>批次·档期</span><strong>${esc(meta.batch_number || "未分批")} · ${fmt(schedule)}</strong></div><div><span>状态</span>${coverageStatus}</div><div><span>时间</span><strong>${fmtUtc(group.createdAt)}</strong></div><div class="row-actions">${meta.package_id ? `<button class="button button-secondary button-small" type="button" data-action="view-media-package" data-id="${esc(meta.package_id)}">查看运营包</button>` : ""}${group.assets[0] ? `<button class="button button-secondary button-small" type="button" data-action="reveal-media-asset" data-id="${esc(group.assets[0].id)}">${icon("folder-open")} 打开文件夹</button>` : ""}</div></div><div class="media-gallery-assets">${assetsMarkup}</div></article>`;
  }

  function renderMediaBatchProgress() {
    const target = el("mediaBatchProgress");
    if (!target) return;
    if (!state.mediaBatchId) {
      target.hidden = true;
      target.innerHTML = "";
      return;
    }
    const batch = state.imageBatches.find(item => item.id === state.mediaBatchId);
    const groups = state.mediaGroups.filter(group => group.complete !== null);
    const completed = groups.filter(group => group.complete).length;
    const incomplete = groups.length - completed;
    const missingImages = groups.reduce((total, group) => total + group.missingRoles.length, 0);
    const expectedPackages = Number(batch?.package_count || groups.length);
    const allComplete = groups.length === expectedPackages && incomplete === 0;
    const status = allComplete
      ? `<span class="media-batch-status is-complete">全部完成</span>`
      : `<button class="media-batch-status is-incomplete" type="button" data-action="show-missing-media">待补 ${incomplete} 个运营包 · ${missingImages} 张图</button>`;
    target.hidden = false;
    target.innerHTML = `<div><span>当前批次</span><strong>${esc(batch?.batch_number || "未命名批次")}</strong></div><div><span>工单</span><strong>${expectedPackages}</strong></div><div><span>图片齐全</span><strong>${completed}</strong></div><div><span>未完成</span><strong>${incomplete}</strong></div>${status}`;
  }

  function jumpToMissingMediaGroup(packageId = "") {
    const targetGroup = packageId
      ? state.mediaGroups.find(group => group.meta.package_id === packageId && group.complete === false)
      : window.ZhijuMediaGallery.nextIncompleteGroup(state.mediaGroups, state.mediaMissingPackageId);
    if (!targetGroup) return;
    state.mediaPackageId = "";
    state.mediaLanguage = "";
    state.mediaChannel = "";
    state.mediaStatus = "incomplete";
    state.mediaMissingPackageId = targetGroup.meta.package_id;
    const filteredGroups = window.ZhijuMediaGallery.filterGroups(state.mediaGroups, { status: state.mediaStatus });
    const targetIndex = filteredGroups.indexOf(targetGroup);
    state.mediaPage = Math.floor(targetIndex / MEDIA_GROUPS_PER_PAGE) + 1;
    renderMediaGallery();
    ...
    notify(`${targetGroup.meta.video_id || targetGroup.meta.drama_code || targetGroup.meta.chinese_title} 缺少：${targetGroup.missingRoles.join("、")}`, true);
  }

  function renderMediaGallery() {
    const languageSelect = el("mediaLanguageFilter");
    const channelSelect = el("mediaChannelFilter");
    const statusSelect = el("mediaStatusFilter");
    const content = el("mediaGalleryContent");
    const summary = el("mediaGallerySummary");
    const pagination = el("mediaGalleryPagination");
    if (!languageSelect || !channelSelect || !statusSelect || !content || !summary || !pagination) return;
    ...
    const filteredGroups = window.ZhijuMediaGallery.filterGroups(languageGroups, {
      channel: state.mediaChannel,
      status: state.mediaStatus,
    });
    statusSelect.value = state.mediaStatus;
    const page = window.ZhijuMediaGallery.paginateGroups(filteredGroups, state.mediaPage, MEDIA_GROUPS_PER_PAGE);
    state.mediaPage = page.page;
    state.visibleMediaGroups = page.items;
    const imageCount = filteredGroups.reduce((total, group) => total + group.assets.length, 0);
    summary.textContent = `${filteredGroups.length} 个剧目 · ${imageCount} 张可验收图片 · 第 ${page.page}/${page.totalPages} 页`;
    renderMediaBatchProgress();
    ...
    content.innerHTML = state.visibleMediaGroups.length
      ...
      : empty("没有符合条件的素材", "请调整语言、频道或状态筛选条件。");
    pagination.innerHTML = page.totalPages > 1
      ? `<button class="button button-secondary button-small" type="button" data-action="media-page" data-page="${page.page - 1}" ${page.page === 1 ? "disabled" : ""}>上一页</button><span>第 ${page.page} / ${page.totalPages} 页</span><button class="button button-secondary button-small" type="button" data-action="media-page" data-page="${page.page + 1}" ${page.page === page.totalPages ? "disabled" : ""}>下一页</button>`
      : "";
    renderIcons();
  }

  async function loadMediaGalleryAssets() {
    const [assets, packages] = await Promise.all([
      api(`/media-assets${query({ limit: 500, batch_id: state.mediaBatchId })}`),
      state.mediaBatchId
        ? api(`/image-processing/batches/${encodeURIComponent(state.mediaBatchId)}/asset-coverage`)
        : Promise.resolve(state.mediaContexts),
    ]);
    state.mediaAssets = assets;
    state.mediaCoverage = state.mediaBatchId ? packages : [];
    state.mediaGroups = buildMediaGroups(assets, packages, state.channels);
    state.mediaPage = 1;
    ...
    el("mediaLanguageFilter").innerHTML = `<option value="">全部语言</option>${languages.map(code => `<option value="${esc(code)}" ${code === state.mediaLanguage ? "selected" : ""}>${esc(languageLabel(code))}</option>`).join("")}`;
    renderMediaGallery();
  }
  ...
  function renderMediaRunHistory() {
    const target = el("mediaRunHistory");
    const subtitle = el("mediaRunHistorySubtitle");
    const actions = el("mediaRunHistoryActions");
    const pagination = el("mediaRunHistoryPagination");
    if (!target || !subtitle || !actions || !pagination) return;
    const runs = state.imageRuns;
    const total = state.imageRunHistoryTotal;
    const totalPages = state.mediaRunHistoryExpanded ? Math.max(1, Math.ceil(total / 10)) : 1;
    subtitle.textContent = `${total} 次${state.mediaBatchId ? "当前批次" : "全部批次"}导入记录 · ${state.mediaRunHistoryExpanded ? `第 ${state.mediaRunHistoryPage}/${totalPages} 页` : "默认显示最近 1 条"}`;
    actions.innerHTML = total > 1
      ? `<button class="button button-secondary button-small" type="button" data-action="toggle-media-run-history">${state.mediaRunHistoryExpanded ? `${icon("chevron-up")} 收起` : `${icon("history")} 查看全部历史（${total}）`}</button>`
      : "";
    const rows = runs.map(run => `<tr><td><span class="cell-main mono">${esc(run.batch_number)}</span><span class="cell-sub">${fmtUtc(run.created_at)}</span></td><td>${tag(run.status)}</td><td>${run.total_files}</td><td>${run.matched_files}</td><td>${run.unmatched_files}</td><td>${run.generated_files}</td><td><div class="row-actions">${run.matched_files ? `<button class="button button-primary button-small" data-action="generate-run-logo" data-id="${run.id}">${icon("stamp")} 生成 Logo 图</button>` : ""}<button class="icon-button" title="查看处理明细" aria-label="查看处理明细" data-action="image-run-detail" data-id="${run.id}">${icon("list-tree")}</button></div></td></tr>`);
    target.innerHTML = rows.length
      ? table(["批次", "状态", "导入", "已匹配", "未匹配", "Logo 成品", ""], rows, 920)
      : empty("当前批次还没有处理记录", "导入所选批次的图片后，记录会显示在这里。");
    pagination.innerHTML = state.mediaRunHistoryExpanded && totalPages > 1
      ? `<button class="button button-secondary button-small" type="button" data-action="media-run-history-page" data-page="${state.mediaRunHistoryPage - 1}" ${state.mediaRunHistoryPage === 1 ? "disabled" : ""}>上一页</button><span>第 ${state.mediaRunHistoryPage} / ${totalPages} 页</span><button class="button button-secondary button-small" type="button" data-action="media-run-history-page" data-page="${state.mediaRunHistoryPage + 1}" ${state.mediaRunHistoryPage === totalPages ? "disabled" : ""}>下一页</button>`
      : "";
    renderIcons();
  }

  async function loadMediaRunHistory() {
    const page = await window.ZhijuMediaGallery.loadRunHistory(api, {
      batchId: state.mediaBatchId,
      expanded: state.mediaRunHistoryExpanded,
      page: state.mediaRunHistoryPage,
    });
    state.imageRuns = page.items;
    state.imageRunHistoryTotal = page.total;
    state.mediaRunHistoryPage = page.page;
    renderMediaRunHistory();
  }

  async function showMedia() {
    const [workspace, batches] = await Promise.all([api("/settings/image-workspace"), api("/image-processing/batches")]);
    state.imageBatches = batches;
    state.mediaRunHistoryExpanded = false;
    state.mediaRunHistoryPage = 1;
    if (state.mediaBatchId && !batches.some(batch => batch.id === state.mediaBatchId)) state.mediaBatchId = "";
    ...
    const runHistory = section("处理历史", '<span id="mediaRunHistorySubtitle"></span>', '<div id="mediaRunHistory"></div><div class="pagination" id="mediaRunHistoryPagination"></div>', '<div id="mediaRunHistoryActions"></div>', true);
    ...
    renderIcons();
    await loadMediaRunHistory();
    try {
      const [contexts, channels] = await Promise.all([api("/media-assets/contexts"), api("/channels/overview")]);
      state.mediaContexts = contexts;
      state.channels = channels;
      await loadMediaGalleryAssets();
    } catch (error) {
      el("mediaGallerySummary").textContent = "素材列表读取失败";
      el("mediaGalleryContent").innerHTML = empty("素材读取失败", error.message);
      notify(error.message, true);
    }
  }
  ...
  async function showSettings() {
    ...
    if (state.settingsTab === "cadence") {
      ...
      const environmentSwitch = data.can_switch_environment ? `<section class="environment-switch-panel"><div><strong>数据库环境</strong><span>代码机可直接切换；切换后立即影响后续读取和写入，重启后按本机启动配置连接。</span></div><div class="environment-segmented" role="group" aria-label="数据库环境"><button class="environment-segment ${data.environment === "development" ? "is-active" : ""}" data-action="switch-database-environment" data-environment="development" ${data.environment === "development" ? "disabled" : ""}>开发环境</button><button class="environment-segment ${data.environment === "production" ? "is-active is-production" : ""}" data-action="switch-database-environment" data-environment="production" ${data.environment === "production" ? "disabled" : ""}>生产环境</button></div></section>` : "";
  ...
  document.addEventListener("click", async event => {
    ...
    try {
      if (action === "add-channel") openModal("新增频道", channelForm());
      ...
      else if (action === "apply-drama-library-filters") {
        state.dramaLibrarySearch = el("dramaLibrarySearch")?.value.trim() || "";
        state.dramaLibraryStatus = el("dramaLibraryStatus")?.value || "";
        state.dramaLibraryBatch = el("dramaLibraryBatch")?.value.trim() || "";
        state.dramaLibrarySortBy = el("dramaLibrarySortBy")?.value || "drama_number";
        state.dramaLibrarySortOrder = el("dramaLibrarySortOrder")?.value || "asc";
        state.dramaLibraryPageSize = Number(el("dramaLibraryPageSize")?.value || 50);
        state.dramaLibraryPage = 1;
        await loadView("dramas", { preservePosition: true });
      }
      else if (action === "drama-library-page") { state.dramaLibraryPage = Number(button.dataset.page); await loadView("dramas"); }
      else if (action === "sync-feishu-dramas") {
        button.disabled = true;
        const result = await api("/feishu-sync/dramas", { method: "POST" });
        notify(`飞书剧库同步完成：剧目新增 ${result.rows_inserted}，更新 ${result.rows_updated}；集数/时长新增 ${result.metadata_rows_inserted || 0}，更新 ${result.metadata_rows_updated || 0}`);
        await loadView("dramas", { preservePosition: true });
      }
      else if (action === "go-drama-progress") await loadView("dramaProgress");
      else if (action === "back-drama-library") await loadView("dramas");
      else if (action === "apply-drama-progress-filters") {
        state.dramaProgressSearch = el("dramaProgressSearch")?.value.trim() || "";
        state.dramaProgressBatch = el("dramaProgressBatch")?.value.trim() || "";
        state.dramaProgressStatus = el("dramaProgressStatus")?.value || "";
        state.dramaProgressNode = el("dramaProgressNode")?.value || "";
        state.dramaProgressSortOrder = el("dramaProgressSortOrder")?.value || "asc";
        state.dramaProgressPageSize = Number(el("dramaProgressPageSize")?.value || 50);
        state.dramaProgressPage = 1;
        await loadView("dramaProgress", { preservePosition: true });
      }
      else if (action === "drama-progress-page") { state.dramaProgressPage = Number(button.dataset.page); await loadView("dramaProgress"); }
      else if (action === "sync-zhihe-progress") {
        button.disabled = true;
        const result = await api("/integrations/zhihe/drama-progress/sync", { method: "POST" });
        notify(`智核进度同步完成：获取 ${result.fetched}，更新 ${result.updated}，旧数据 ${result.skipped_stale}，未匹配 ${result.skipped_unmatched}`);
        await loadView("dramaProgress", { preservePosition: true });
      }
        ...
        button.disabled = true;
        const result = await api("/feishu-sync/drama-languages", { method: "POST" });
        notify(`飞书语言同步完成：新增 ${result.rows_inserted}，更新 ${result.rows_updated}，跳过 ${result.rows_skipped}`);
        await showDramaDetail(id, "languages");
      }
      else if (action === "toggle-drama-language") {
        const languageId = button.dataset.languageId;
        if (button.dataset.covered === "1") await api(`/dramas/${id}/languages/${languageId}`, { method: "DELETE" });
        else await api(`/dramas/${id}/languages/${languageId}`, { method: "PUT", body: JSON.stringify({ translation_status: "ready", asset_status: "ready" }) });
        await showDramaDetail(id, "languages");
      }
      ...
      else if (action === "edit-schedule-video") {
        const item = [...(state.schedules || []), ...(state.scheduleFull?.items || [])].find(candidate => candidate.schedule_id === id);
        if (item) openSourceVideoForm("schedule", item);
      }
      else if (action === "edit-task-video") {
        const item = state.tasks.find(candidate => candidate.task_id === id);
        if (item) openSourceVideoForm("task", item);
      }
      else if (action === "schedule-view-mode") {
        state.scheduleViewMode = ["day", "full", "all-day"].includes(button.dataset.mode) ? button.dataset.mode : "day";
      ...
      else if (action === "open-media-viewer") {
        state.mediaViewer = { groupIndex: Number(button.dataset.groupIndex), assetIndex: Number(button.dataset.assetIndex) };
        renderMediaViewer();
      }
      ...
      else if (action === "select-media-viewer") {
        if (state.mediaViewer) {
          state.mediaViewer.assetIndex = Number(button.dataset.assetIndex);
          renderMediaViewer();
        }
      }
      ...
      else if (action === "show-missing-media") jumpToMissingMediaGroup();
      else if (action === "show-media-package-missing") jumpToMissingMediaGroup(id);
      else if (action === "media-page") {
        state.mediaPage = Number(button.dataset.page);
        renderMediaGallery();
        el("mediaGallerySummary")?.scrollIntoView({ block: "start", behavior: "auto" });
      }
      else if (action === "toggle-media-run-history") {
        state.mediaRunHistoryExpanded = !state.mediaRunHistoryExpanded;
        state.mediaRunHistoryPage = 1;
        await loadMediaRunHistory();
      }
      else if (action === "media-run-history-page") {
        state.mediaRunHistoryPage = Number(button.dataset.page);
        await loadMediaRunHistory();
        el("mediaRunHistorySubtitle")?.scrollIntoView({ block: "center", behavior: "auto" });
      }
  ...
  document.addEventListener("change", async event => {
    ...
    if (event.target.id === "mediaBatchFilter") {
      state.mediaBatchId = event.target.value;
      state.mediaPackageId = "";
      state.mediaLanguage = "";
      state.mediaChannel = "";
      state.mediaStatus = "";
      state.mediaMissingPackageId = "";
      state.mediaPage = 1;
      state.mediaRunHistoryExpanded = false;
      state.mediaRunHistoryPage = 1;
      el("mediaRunHistorySubtitle").textContent = "正在读取所选批次历史…";
      el("mediaRunHistory").innerHTML = `<div class="loading-inline">处理历史加载中</div>`;
      el("mediaGallerySummary").textContent = "正在读取所选批次素材…";
      el("mediaGalleryContent").innerHTML = `<div class="loading-inline">素材列表加载中</div>`;
      try { await Promise.all([loadMediaRunHistory(), loadMediaGalleryAssets()]); } catch (error) { notify(error.message, true); }
    }
    ...
    if (event.target.id === "mediaStatusFilter") { state.mediaStatus = event.target.value; state.mediaPage = 1; renderMediaGallery(); }
  ...
  document.addEventListener("submit", async event => {
    ...
    try {
      ...
      if (form.id === "imageImportForm") {
        if (form.dataset.busy === "true") return;
        const files = window.ZhijuMediaGallery.filterImportFiles([
          ...form.elements.folder_files.files,
          ...form.elements.image_files.files,
        ]);
        if (!files.length) throw new Error("请选择图片；.DS_Store 已自动忽略");
        ...
        try {
          ...
          notify(run.unmatched_files
            ? `分类完成：匹配 ${run.matched_files}，未匹配 ${run.unmatched_files}（图片已保留，可在处理明细查看）`
            : `分类完成：匹配 ${run.matched_files}，未匹配 0`);
          await loadView("media", { preservePosition: true });
        } catch (error) {
      ...
      if (form.id === "dramaForm") {
        const dramaId = data.drama_id; delete data.drama_id;
        data.aliases = data.aliases.split(/[,，]/).map(x => x.trim()).filter(Boolean);
        data.core_terms = data.core_terms.split(/[,，]/).map((term, index) => ({ term_type: "keyword", term: term.trim(), weight: Math.max(.1, 1 - index * .1), source: "manual" })).filter(x => x.term);
        for (const key of ["batch_name", "baidu_cloud_url", "content_summary", "plot_archive", "plot_pattern", "core_personas"]) if (!data[key]) data[key] = null;
        data.expires_at = data.expires_at ? `${data.expires_at}T23:59:59` : null;
        await api(dramaId ? `/dramas/${dramaId}` : "/dramas", { method: dramaId ? "PATCH" : "POST", body: JSON.stringify(data) });
        notify(dramaId ? "剧目资料已更新" : "剧目已入库"); closeModal(); closeDrawer(); await loadView("dramas", { preservePosition: true });
      }
      ...
      if (form.id === "dramaProgressForm") {
        const dramaId = data.drama_id; delete data.drama_id;
        data.episode_count = data.episode_count === "" ? null : Number(data.episode_count);
        data.total_duration_seconds = data.total_duration_seconds === "" ? null : Number(data.total_duration_seconds);
        data.last_error = data.last_error.trim() || null;
        await api(`/dramas/${dramaId}/production-state`, { method: "PUT", body: JSON.stringify(data) });
        notify("制剧进度已保存"); closeModal();
      ...
      if (form.id === "sourceVideoForm") {
        const entityType = data.entity_type, entityId = data.entity_id;
        const payload = { source_video_url: data.source_video_url.trim() || null, source_video_id: data.source_video_id.trim() || null };
        await api(`/${entityType === "task" ? "tasks" : "schedules"}/${entityId}/source-video`, { method: "PATCH", body: JSON.stringify(payload) });
        notify("视频信息已更新，排期与工单已联动"); closeModal(); await loadView(state.view, { preservePosition: true });
      }
  ...
  document.addEventListener("keydown", event => {
    ...
    const libraryFilter = event.target.closest?.("#dramaLibrarySearch, #dramaLibraryBatch");
    const progressFilter = event.target.closest?.("#dramaProgressSearch, #dramaProgressBatch");
    const action = libraryFilter ? "apply-drama-library-filters" : progressFilter ? "apply-drama-progress-filters" : "";
    if (!action) return;
    event.preventDefault();
